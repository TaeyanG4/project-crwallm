...
브라우저가 편하면 `crwallm serve` 뒤 `localhost:8000` — **같은 화면입니다.**
...
[0. 창](#0-창)의 화면은 브라우저에서도 그대로 뜹니다. **같은 HTML 한 벌**을
창은 파일에서 읽고 브라우저는 API에서 받습니다.

```bash
crwallm serve        # 그리고 localhost:8000
```

창에서는 `window.pywebview.api`, 브라우저에서는 같은 출처의 `/api/ui/*`를
부르고, 둘 다 `services/quick.py` 하나를 지나갑니다. **포트는 하나입니다** —
화면을 API가 직접 내보내니 앞단에 프록시도 Node도 없습니다.

브라우저 쪽만 다른 점이 둘 있습니다. **저장**은 네이티브 대화상자 대신
브라우저 다운로드이고, **진행 상황**은 실시간으로 흐르지 않습니다(창은
흐릅니다). 탭마다 별개의 작업으로 취급되니 두 탭이 서로를 덮어쓰지 않습니다.
...
API·워커·Next.js가 함께 뜨고 브라우저가 열립니다. Ctrl-C로 전부 내려갑니다.
**여기서 처음으로 Docker와 Node가 필요해집니다.**

| 플래그 | |
|---|---|
| `--web` | 예전 Next.js 앱도 함께 띄움 (기본은 끔) |
| `--no-worker` | 잡을 큐에만 넣고 실행하지 않음 |
| `--no-open` | 브라우저를 열지 않음 |
| `--port` / `--ui-port` | 포트 변경 |

**워커가 없으면 UI가 잡을 큐에 넣고 아무 일도 일어나지 않습니다** — 화면에는
`대기` 상태로만 보이고 이유가 나오지 않습니다. `up`이 함께 띄우는 이유가
그것입니다.
