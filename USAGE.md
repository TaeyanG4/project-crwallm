...
**레시피 탭에도 같은 두 버튼이 있습니다** — 확인만 해보기, 확인하고 활성화.
어느 항목이 몇 % 채워졌는지까지 나오니, 사이트가 개편돼 어떤 셀렉터가 죽었는지
바로 보입니다. 만드는 것(`adapt`)만 터미널에 남아 있습니다 — 모델이 여러 후보를
만들어 채점하는 몇 분짜리 일이라서입니다.
...
| 탭 | 하는 일 | 필요한 것 |
|---|---|---|
| **모으기** | [0. 창](#0-창)과 같은 흐름 + 자세한 설정 | 없음 |
| **작업** | 큐에 넣고 라이브로 보기, 모은 것·가져온 쪽·내보내기 | Docker |
| **레시피** | 목록·상세, **확인**과 **활성화** | 없음 |
| **대화** | 문장으로 시키기 | Ollama |
| **설정** | 모델, 화면 기본값, 서버 설정 | 없음 |

화면 위 오른쪽에 **데이터베이스**와 **모델**이 켜졌는지 나옵니다. 꺼져 있으면
그 탭만 비고 나머지는 그대로 됩니다.
...
모든 화면에서 **자세한 설정**을 펴면 CLI와 같은 것을 만집니다.

| 조절 | 모으기 | 작업 |
|---|---|---|
| 최대 쪽수 · 최대 단계 | ✓ | ✓ |
| 가져오는 방식 (http / auto / browser) | ✓ | ✓ |
| 동시 요청 · 호스트당 동시 | ✓ | ✓ |
| 한 요청 간격 (ms) | ✓ | ✓ |
| 응답 대기 (초) | — | ✓ |
| 스크롤 횟수 | ✓ | ✓ |
| 볼 주소 / 뺄 주소 (정규식) | ✓ | ✓ |
| 레시피 · 허용 도메인 · 우선순위 · 훑기 모드 | — | ✓ |

기본값은 **설정** 탭에서 한 번 정해두면 두 화면이 그 값으로 시작합니다.
이 브라우저에만 저장되고, 서버 설정과 섞이지 않습니다.

### 설정 탭

- **모델** — 설치된 것과 받을 수 있는 것. **이걸로**로 고르면 `.env`에 적히고,
  이 창은 즉시, 워커는 다시 시작할 때 반영됩니다. **받기**는 진행률이 보입니다.
  이 기기에 큰 모델은 받기 전에 그렇다고 알려줍니다.
- **기본값** — 위 표의 시작값.
- **서버 설정** — 읽기 전용입니다. 주소·토큰·데이터베이스는 `.env`가 정하고,
  화면에서 고칠 수 있게 하면 지금 쓰는 창을 스스로 잠글 수 있습니다.
  비밀번호와 토큰은 값 대신 `***`와 `설정됨`으로 나옵니다.
