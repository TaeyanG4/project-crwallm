...
def create_app() -> FastAPI:
    ...
    async def robots(request: Request) -> str:
        """Rules that are ignored, plus a pointer that is not.

        The crawler does not obey robots.txt (docs/17_NON_GOALS.md) but this is
        still where a site says where its sitemaps are, and reading it for that
        alone is the cheapest way to find them.
        """
        origin = f"{request.url.scheme}://{request.url.netloc}"
        return (
            "User-agent: *\n"
            "Disallow: /shop\n"  # deliberately covers the listing we do crawl
            "Crawl-delay: 10\n"
    ...
    async def sitemap_index(request: Request) -> Response:
        ...
        body = (
            ...
            + "".join(
                f"<sitemap><loc>{origin}/sitemap-{n}.xml</loc>"
                f"<lastmod>2026-0{n}-01</lastmod></sitemap>"
                for n in (1, 2)
            )
            + "</sitemapindex>"
        )
    ...
    @app.get("/sitemap-{shard}.xml")
    async def sitemap_shard(shard: int, request: Request) -> Response:
        origin = f"{request.url.scheme}://{request.url.netloc}"
        start = (shard - 1) * 5 + 1
        body = (
            '<?xml version="1.0" encoding="UTF-8"?>'
            ...
            + "".join(
                f"<url><loc>{origin}/shop/item/{i}</loc>"
                f"<lastmod>2026-0{shard}-1{i % 9}</lastmod>"
                f"<priority>0.{9 - (i % 5)}</priority></url>"
    ...
    @app.get("/sitemap-broken.xml")
    async def sitemap_broken() -> Response:
        """Generated by a plugin having a bad day. Should cost the sitemap,
        not the crawl."""
        return Response(content="<urlset><url><loc>unclosed", media_type="application/xml")

    # ------------------------------------------------- soft 404s and clones

    @app.get("/ghost/{n}", response_class=HTMLResponse)
    async def ghost(n: int) -> str:
        """An infinite family of 200s that all mean "not found".

        A status code alone would let a crawler collect four hundred of these.
        """
        return (
            "<html><head><title>Not found</title></head><body>"
            "<h1>Page not found</h1><p>The page does not exist.</p></body></html>"
        )

    @app.get("/mirror/{n}", response_class=HTMLResponse)
    async def mirror(n: int) -> str:
        """Distinct URLs, near-identical content.

        Only the timestamp differs, which is exactly the case an exact hash
        misses and simhash catches.
        """
        return (
            "<html><body><article>"
            "<h1>The same article every time</h1>"
            "<p>A long enough body of text that the deduper will consider it, "
            "with several sentences so the shingles are meaningful. The content "
            "does not change between these URLs; only the footer does.</p>"
            f"<footer>rendered at request {n}</footer>"
            "</article></body></html>"
        )
